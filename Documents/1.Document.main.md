...
###### 1.1.3.4. Monitoring & Logging

**Monitoring and Logging** in Azure help to track performance, detect issues, and debug problems

**a. Azure Monitor**

- Azure Monitor is the central platform in Azure for monitoring performance and health of applications, infrastructure and services
  | Type | Description | Example |
  | ------- | ----------------------------------- | ------------------------------ |
  | Metrics | Numeric values over time | CPU %, memory usage |
  | Logs | Text-based, structured/unstructured | App logs, request logs, errors |

**b. Application Insights**

- **Application Insights** is a part of **Azure Monitor**, focused on monitoring your application’s behavior in real time.
- Basically, in the Dotnet application we just need to install SDK and use this application insight service and use built-in logger to be able to integrate with Application Insight, and it will track:
  - Requests (URLs, duration, result codes)
  - Exceptions (errors with stack trace)
  - Dependencies (SQL, HTTP calls)
  - Performance (response time, memory, etc.)
  - Custom events & logs

**c. Log Analytics (Kusto Query Language - KQL)Log Analytics (Kusto Query Language - KQL)**

Azure provides a unified monitoring system called Azure Monitor, which collects and analyzes data from:

- **Applications** (.NET)
- **Infrastructure** (VMs, App Services, etc.)
- **Azure Platform itself**

Azure Monitor contains two main types of data:

- **Metrics** → numeric, time-series data (CPU %, memory, etc.)
- **Logs** → event-based data (requests, errors, diagnostics)

| Scenario                                   | What Gets Logged                         | Where It’s Stored                              |
| ------------------------------------------ | ---------------------------------------- | ---------------------------------------------- |
| **1. VM only (no agent)**                  | Activity logs, basic metrics             | Azure Activity Log (CPU, Memory, Disk)         |
| **2. VM + Monitoring Agent**               | OS events, performance metrics           | Log Analytics Workspace                        |
| **3. .NET App + Application Insights**     | Requests, dependencies, exceptions       | Application Insights (stored in Log Analytics) |
| **4. Linked App Insights + Log Analytics** | Full visibility (app + infra + platform) | Log Analytics Workspace                        |

**d. Azure Diagnostic Settings**
By default, Azure only keeps limited logs internally (short retention, not queryable). With Diagnostic Settings, you can:

- Centralize all logs (VM, SQL, Function, etc.)
- Analyze them with Log Analytics / KQL
- Keep long-term history (Storage)
- Stream to external tools (Event Hub, SIEM)

**e. Alerts**
Azure Alerts automatically notify you or trigger an action when certain conditions are met — like high CPU usage, failed requests, or specific log patterns.

**Types of Alerts**
| Type | Source | Example | Typical Use |
| ----------------------- | ------------------------------------------------ | ---------------------------- | ----------------------------- |
| **Metric Alerts** | Real-time metrics (CPU %, memory, request count) | CPU > 80% for 5 min | Performance / availability |
| **Log Alerts** | Query results from Log Analytics (KQL) | Count of errors > 100 | Application / diagnostic logs |
| **Activity Log Alerts** | Azure platform events | Resource deleted, VM stopped | Security / governance |
| **Prometheus Alerts** | From AKS (Kubernetes metrics) | Pod crash loop | AKS workload monitoring |

#### 1.2. Public, Private, Hybrid Cloud

##### 1.2.1. What Are Cloud Deployment Models?

A **cloud deployment model** defines where the cloud resources are hosted, who manages them, and who access to them, basically it is the cloud infrastructure is set up and who controls what

**The 4 Main Cloud Deployment Models**

| Model             | Ownership                                                                            | Location                                       | Typical Use Case                                                  |
| ----------------- | ------------------------------------------------------------------------------------ | ---------------------------------------------- | ----------------------------------------------------------------- |
| **Public Cloud**  | Owned & operated by a third-party provider (like Microsoft Azure, AWS, Google Cloud) | Shared across multiple tenants (organizations) | Startups, SaaS apps, dynamic scaling workloads                    |
| **Private Cloud** | Dedicated to a single organization                                                   | On-premises or hosted by a provider            | Large enterprises needing control, compliance, or security        |
| **Hybrid Cloud**  | Combination of public + private clouds                                               | Mix of on-prem and cloud                       | Workloads needing both control (private) and scalability (public) |
| **Multi-Cloud**   | Using multiple public cloud providers                                                | Across several vendors                         | Avoid vendor lock-in, or use best services from each provider     |

##### 1.2.2. Public Cloud

A **public cloud** is a cloud model where services (like compute, storage, networking, database,...) are owned and managed by a third-party cloud provider (like Azure, AWS, Google Cloud) and shared among multiple customers (tenants) over the internet

##### 1.2.3. Private Cloud

Understand environments exclusive to one organization, managed internally or via Azure hybrid tech.

##### 1.2.4. Hybrid Cloud

Learn how Azure connects on-prem resources with public cloud to form a unified environment.

##### 1.2.5. Multi-Cloud (Bonus for AZ-204 developers)
